...
if [ ! -d "src/agent_harness" ]; then
  ...
  echo "  bash patch_agent_harness_v20_browser_tools.sh"
  exit 1
fi

BACKUP_DIR="patch_backup_v20_browser_tools_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BACKUP_DIR"

for f in max_cli.py browser_flow.py; do
...
cat > src/agent_harness/browser_flow.py <<'PY'
...
import_line = "from .browser_flow import browser_project as direct_browser_project"
...
'''
    if marker not in text:
        marker = '    "read": {'
    if marker not in text:
        raise SystemExit("Could not find COMMANDS insertion point for browser.")
    text = text.replace(marker, entry + marker, 1)

if "def _direct_browser(" not in text:
...
branch = '''    if argv and argv[0] in {"browser", "chromium"}:\n        return _direct_browser(argv[1:])\n\n'''
...
if '("max browser screenshot https://example.com", "Save a rendered page screenshot")' not in text:
    text = text.replace(
...
echo ""
echo "v0.20 browser tools installed."
echo ""
echo "Enable browser:"
echo "  max config set allow_browser true"
echo ""
echo "Try:"
echo "  max browser check"
echo "  max browser text https://example.com"
echo "  max browser screenshot https://example.com example.png"
echo "  max browser history"
